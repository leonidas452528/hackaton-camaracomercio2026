...
export type EvidenceStatus =
  | 'documentado' | 'referencia reglamentaria (verificar)' | 'aproximado'
  | 'ilustrativo' | 'POR MEDIR' | 'SIMULADO' | 'calculado';
...
function evidence<T>(value: T, status: EvidenceStatus, source: string, note = ''): Evidence<T> {
...
export const sources = {
  ...
  hockey: 'https://www.fih.hockey/static-assets/pdf/fih-junior_world-cup-_events_field_specifications-16-01-05.pdf',
...
export const site = {
  ...
  coordinates: {
    metersPerUnit: 1,
    eastAxis: '+X', northAxis: '-Z', upAxis: '+Y',
    legend: 'Posiciones aproximadas — orientación por verificar',
  },
  venues: {
    hockey: {
      name: 'Cancha de Hockey Miguel Calero',
      position: evidence([0, 0, 0] as const, 'aproximado', sources.prompt, 'Origen relativo de la escena.'),
      field: evidence({ length: 91.4, width: 55 }, 'referencia reglamentaria (verificar)', sources.hockey,
        'Dimensiones tomadas de la propuesta; confirmar las medidas reales del escenario.'),
      footprintWithMargins: evidence({ length: 97.4, width: 59 }, 'referencia reglamentaria (verificar)', sources.hockey),
    },
    volleyball: {
      name: 'Coliseo de Voleibol Francisco Chois',
      position: evidence([148, 0, -111] as const, 'aproximado', sources.prompt, 'Error de decenas de metros.'),
      court: evidence({ length: 18, width: 9 }, 'referencia reglamentaria (verificar)', sources.volleyball),
      freeZonePerSide: evidence(3, 'referencia reglamentaria (verificar)', sources.volleyball),
      minimumClearHeight: evidence(7, 'referencia reglamentaria (verificar)', sources.volleyball),
      modeledCourtCount: evidence(1, 'ilustrativo', sources.prompt, 'No representa el número real de canchas.'),
      actualCourtCount: evidence<number | null>(null, 'POR MEDIR', sources.proposal),
      actualFootprint: evidence<readonly [number, number] | null>(null, 'POR MEDIR', sources.proposal),
      actualHeight: evidence<number | null>(null, 'POR MEDIR', sources.proposal),
      roofArea: evidence<number | null>(null, 'POR MEDIR', sources.proposal),
    },
    baseball: {
      name: 'Diamante de Béisbol · centro de acopio',
      position: evidence([173, 0, 40] as const, 'aproximado', sources.prompt, 'Error de decenas de metros.'),
      placeholderFootprint: evidence({ length: 60, width: 60 }, 'ilustrativo', sources.prompt, 'Dimensiones reales POR MEDIR.'),
    },
    evangelista: {
      name: 'Coliseo Evangelista Mora · recuadro aparte',
      position: evidence([-195, 0, -726] as const, 'aproximado', sources.prompt, 'Fuera del encuadre principal.'),
      actualFootprint: evidence<readonly [number, number] | null>(null, 'POR MEDIR', sources.proposal),
      actualHeight: evidence<number | null>(null, 'POR MEDIR', sources.proposal),
      actualCourtCount: evidence<number | null>(null, 'POR MEDIR', sources.proposal),
  ...
  equipment: {
    partition: evidence({ length: 2, width: 2, height: 2 }, 'ilustrativo', sources.prompt),
    kit: evidence({ people: 20, partitions: 5, footprintM2: 70 }, 'ilustrativo', sources.prompt,
      'Huella de referencia con circulación; las particiones no acreditan área individual suficiente.'),
    canopy: evidence({ length: 10, width: 7, clearHeight: 3 }, 'ilustrativo', sources.prompt),
    closedModule: evidence({ length: 5, width: 3.5, height: 2.5 }, 'ilustrativo', sources.prompt),
    closedModuleLabels: ['Espacio NNA', 'Punto de salud'],
    toilet: evidence({ length: 1.2, width: 1.2, height: 2.3 }, 'ilustrativo', sources.prompt),
    tank: evidence({ length: 1, width: 1, height: 1.2 }, 'ilustrativo', sources.prompt,
    ...
    solarKitCount: evidence(1, 'ilustrativo', sources.prompt, 'Kit compartido; potencia y dimensiones por definir.'),
  },
  scenario: {
    people: evidence(143, 'documentado', sources.official, 'Caso citado en la ficha; no es ocupación actual.'),
    peoplePerToilet: evidence(20, 'referencia reglamentaria (verificar)', sources.proposal, 'Referencia humanitaria Esfera; edición pendiente de verificar.'),
    litersPerPersonDay: evidence(15, 'referencia reglamentaria (verificar)', sources.proposal,
      'Parámetro solicitado para la maqueta de uso no potable; no acredita que Esfera destine toda esta dotación a ese uso.'),
    simulatedMinorCount: evidence(40, 'SIMULADO', sources.prompt, 'Conteo agregado ficticio, sin registros individuales.'),
  },
  storageFlow: [
    'Lista pública de necesidades', 'Recepción',
    'Clasificación: alimentos, agua, aseo, abrigo, kits',
    'Descarte: vencido o inservible', 'Bodega e inventario',
    'Despacho hacia el refugio', 'Pantalla de trazabilidad de insumos',
  ],
  states: [
...
export const emergencyCalculation = evidence({
  courtLength, courtWidth, courtAreaM2,
  kitsRequired, courtKitsByArea, indoorKits,
